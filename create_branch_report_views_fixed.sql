-- Create views for branch report functionality (FIXED VERSION)
-- This script creates views using only existing columns in the database

-- First, let's check and add any missing columns to branch_collection_performance
DO $$
BEGIN
    -- Add performance_date column if it doesn't exist (might be named period_date)
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_schema = 'kastle_banking' 
                   AND table_name = 'branch_collection_performance' 
                   AND column_name = 'performance_date') THEN
        -- Check if period_date exists and use it
        IF EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_schema = 'kastle_banking' 
                   AND table_name = 'branch_collection_performance' 
                   AND column_name = 'period_date') THEN
            -- Use period_date as our date column
            NULL; -- We'll use period_date in our views
        ELSE
            -- Add performance_date column
...
-- This view uses the actual columns from the base tables
...
SELECT DISTINCT ON (bcp.branch_id)
    ...
    b.manager_id,
    b.is_active,
    COALESCE(bcp.period_date, bcp.performance_date) as performance_date,
    bcp.number_of_accounts as total_cases,
    COALESCE(bcp.number_of_accounts, 0) - COALESCE(bcp.number_of_accounts, 0) / 3 as active_cases, -- Approximation
    COALESCE(bcp.number_of_accounts, 0) / 3 as resolved_cases, -- Approximation
    COALESCE(bcp.total_outstanding, bcp.total_delinquent_amount, 0) as total_outstanding,
    COALESCE(bcp.total_collected_amount, 0) as total_collected,
    bcp.collection_rate,
    COALESCE(bcp.delinquency_rate, 10) as delinquency_rate,
    COALESCE(bcp.avg_dpd, 30) as avg_dpd,
...
ORDER BY bcp.branch_id, COALESCE(bcp.period_date, bcp.performance_date) DESC;
...
SELECT 
    ...
    COALESCE(bcp.period_date, bcp.performance_date) as performance_date,
    bcp.number_of_accounts as total_cases,
    COALESCE(bcp.number_of_accounts, 0) - COALESCE(bcp.number_of_accounts, 0) / 3 as active_cases,
    COALESCE(bcp.number_of_accounts, 0) / 3 as resolved_cases,
    COALESCE(bcp.total_outstanding, bcp.total_delinquent_amount, 0) as total_outstanding,
    COALESCE(bcp.total_collected_amount, 0) as total_collected,
    bcp.collection_rate,
    COALESCE(bcp.delinquency_rate, 10) as delinquency_rate,
...
WHERE b.is_active = true
ORDER BY b.branch_id, COALESCE(bcp.period_date, bcp.performance_date) DESC;
...
SELECT 
    branch_id,
    COALESCE(period_date, performance_date) as performance_date,
    SUM(total_collected_amount) OVER (PARTITION BY branch_id ORDER BY COALESCE(period_date, performance_date) ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) as week_collection,
    SUM(total_collected_amount) OVER (PARTITION BY branch_id ORDER BY COALESCE(period_date, performance_date) ROWS BETWEEN 29 PRECEDING AND CURRENT ROW) as month_collection,
    SUM(total_collected_amount) OVER (PARTITION BY branch_id ORDER BY COALESCE(period_date, performance_date) ROWS BETWEEN 364 PRECEDING AND CURRENT ROW) as year_collection,
    total_collected_amount as daily_collection,
    number_of_accounts as total_cases,
    COALESCE(number_of_accounts, 0) / 3 as resolved_cases,
    collection_rate
FROM kastle_banking.branch_collection_performance
ORDER BY branch_id, COALESCE(period_date, performance_date) DESC;
...
SELECT 
    ...
    b.branch_type,
    b.state as region,
    DATE_TRUNC('month', COALESCE(bcp.period_date, bcp.performance_date)) as month,
    SUM(bcp.total_collected_amount) as monthly_collection,
    SUM(COALESCE(bcp.total_outstanding, bcp.total_delinquent_amount, 0)) as monthly_outstanding,
    AVG(bcp.collection_rate) as avg_collection_rate,
    SUM(bcp.number_of_accounts) as total_cases,
    SUM(COALESCE(bcp.number_of_accounts, 0) / 3) as resolved_cases,
    AVG(COALESCE(bcp.delinquency_rate, 10)) as avg_delinquency_rate,
    COUNT(DISTINCT COALESCE(bcp.period_date, bcp.performance_date)) as days_reported
FROM kastle_banking.branches b
JOIN kastle_banking.branch_collection_performance bcp ON b.branch_id = bcp.branch_id
WHERE b.is_active = true
GROUP BY b.branch_id, b.branch_name, b.branch_type, b.state, DATE_TRUNC('month', COALESCE(bcp.period_date, bcp.performance_date));

...
3. Fixed `collection_buckets` join to use `bucket_id` instead of `id`
4. Properly joined with `customers` table using `customer_id`
5. Removed problematic `auth_user_profiles` join and used `collection_officers` instead
6. Added proper aliases for compatibility with the application code
7. Included calculated fields for priority and status

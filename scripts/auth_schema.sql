...
-- This script creates all necessary tables for user management, roles, permissions,
-- user preferences, templates, and custom dashboard settings
...
CREATE TABLE public.user_role_permissions (
    ...
    user_id UUID NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
    permission_id UUID NOT NULL REFERENCES public.permissions(id) ON DELETE CASCADE,
    granted BOOLEAN DEFAULT true, -- true for grant, false for revoke
    reason TEXT,
...
CREATE POLICY "Users with user:read permission can view all users" ON public.users
    FOR SELECT USING (
        EXISTS (
...
-- Create policies for user_dashboard_widgets

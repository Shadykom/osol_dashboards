...
-- This script inserts sample data for testing the authentication system
...
WHERE r.name = 'manager' 
  AND (
    ...
    (p.resource = 'report' AND p.action NOT IN ('schedule')) OR
...
('Executive Overview', 'High-level executive dashboard for management', 'executive', 
 '{"layout": "grid", "columns": 12, "rows": 8, "gap": 16}',
 '[{"type": "kpi", "position": {"x": 0, "y": 0, "w": 3, "h": 2}}, {"type": "chart", "position": {"x": 3, "y": 0, "w": 6, "h": 4}}, {"type": "table", "position": {"x": 9, "y": 0, "w": 3, "h": 4}}]',
...
('Collection Performance', 'Collection team performance dashboard', 'collection',
 '{"layout": "grid", "columns": 12, "rows": 10, "gap": 16}',
 '[{"type": "metrics", "position": {"x": 0, "y": 0, "w": 12, "h": 2}}, {"type": "heatmap", "position": {"x": 0, "y": 2, "w": 6, "h": 4}}, {"type": "timeline", "position": {"x": 6, "y": 2, "w": 6, "h": 4}}]',
...
('Analytics Dashboard', 'Data analytics and insights dashboard', 'analytics',
 ...
 '[{"type": "filter", "position": {"order": 0}}, {"type": "charts", "position": {"order": 1}}, {"type": "insights", "position": {"order": 2}}]',
 false, (SELECT id FROM public.users WHERE email = 'admin@osol.sa'));

-- Insert user preferences
INSERT INTO public.user_preferences (user_id, preference_key, preference_value, category) VALUES
((SELECT id FROM public.users WHERE email = 'admin@osol.sa'), 'theme', '"dark"', 'appearance'),
((SELECT id FROM public.users WHERE email = 'admin@osol.sa'), 'language', '"ar"', 'localization'),
((SELECT id FROM public.users WHERE email = 'admin@osol.sa'), 'notifications', '{"email": true, "sms": false, "push": true}', 'notifications'),
((SELECT id FROM public.users WHERE email = 'manager@osol.sa'), 'theme', '"light"', 'appearance'),
((SELECT id FROM public.users WHERE email = 'manager@osol.sa'), 'language', '"en"', 'localization'),
((SELECT id FROM public.users WHERE email = 'manager@osol.sa'), 'date_format', '"DD/MM/YYYY"', 'localization');
...
((SELECT id FROM public.users WHERE email = 'admin@osol.sa'), 'My Executive Dashboard', 'Customized executive overview', 
 ...
 '{"layout": "grid", "columns": 12, "rows": 8, "gap": 16, "theme": "dark"}', true),
...
((SELECT id FROM public.users WHERE email = 'manager@osol.sa'), 'Team Performance', 'Collection team performance tracking',
 ...
 '{"layout": "grid", "columns": 12, "rows": 10, "gap": 16, "theme": "light"}', true);
...
((SELECT id FROM public.user_dashboards WHERE name = 'My Executive Dashboard'), 'revenue_kpi',
 '{"title": "Total Revenue", "metric": "revenue", "period": "month", "comparison": "previous_month"}',
 '{"x": 0, "y": 0, "w": 3, "h": 2}', 1),

((SELECT id FROM public.user_dashboards WHERE name = 'My Executive Dashboard'), 'collection_rate',
 '{"title": "Collection Rate", "metric": "collection_rate", "target": 85, "format": "percentage"}',
 '{"x": 3, "y": 0, "w": 3, "h": 2}', 2),

((SELECT id FROM public.user_dashboards WHERE name = 'My Executive Dashboard'), 'trend_chart',
 '{"title": "Collection Trend", "type": "line", "metrics": ["collected", "target"], "period": "last_12_months"}',
 '{"x": 0, "y": 2, "w": 6, "h": 4}', 3);
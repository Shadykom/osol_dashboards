-- Fixed Collection Dashboard Tables Setup
-- This script handles existing table structures properly
...
-- 5. Handle user_roles table - check if it exists first
DO $$
BEGIN
    ...
    ELSE
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
            ...
            ALTER TABLE kastle_banking.user_roles ADD COLUMN user_id VARCHAR(50) UNIQUE;
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
            ...
            ALTER TABLE kastle_banking.user_roles ADD COLUMN full_name VARCHAR(100);
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
            ...
            ALTER TABLE kastle_banking.user_roles ADD COLUMN branch_id VARCHAR(50);
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
            ...
            ALTER TABLE kastle_banking.user_roles ADD COLUMN product_types TEXT[];
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
            ...
            ALTER TABLE kastle_banking.user_roles ADD COLUMN is_active BOOLEAN DEFAULT TRUE;
        END IF;
    END IF;
END $$;
...
-- 8. Create collection_interactions table
...
DO $$
BEGIN
    -- Only add foreign keys if collection_cases table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables 
               WHERE table_schema = 'kastle_banking' 
               AND table_name = 'collection_cases') THEN
        
        -- Add foreign key for remediation_actions
        IF NOT EXISTS (
        ...
        -- Add foreign key for recommended_actions
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
        ...
        -- Add foreign key for promise_to_pay
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
        ...
        -- Add foreign key for collection_interactions
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
...
-- First check what columns exist in user_roles
DO $$
DECLARE
    has_user_id BOOLEAN;
    has_id BOOLEAN;
...
BEGIN
    ...
    -- Insert test user based on table structure
    IF has_user_id THEN
    ...
    ELSIF primary_key_column = 'email' THEN
        -- Email is primary key
        INSERT INTO kastle_banking.user_roles (email, full_name, role, is_active)
        VALUES ('executive@test.com', 'Test Executive', 'EXECUTIVE', true)
        ON CONFLICT (email) DO UPDATE SET role = 'EXECUTIVE';
    ELSE
        -- Use id with email unique
        INSERT INTO kastle_banking.user_roles (email, full_name, role, is_active)
        VALUES ('executive@test.com', 'Test Executive', 'EXECUTIVE', true)
        ON CONFLICT (email) DO UPDATE SET role = 'EXECUTIVE';
    END IF;
...
DO $$
BEGIN
    RAISE NOTICE 'Collection dashboard tables setup completed successfully!';
END $$;
-- Complete Branch Report Setup - All in One Script (FIXED VERSION)
-- This script can be run directly in Supabase SQL editor
-- Fixed to handle the correct relationship: officers -> teams -> branches
...
-- 2. Create a view for branch summary with officer counts (FIXED to handle team relationship)
...
-- 5. Create a view for branch officer performance (FIXED to handle team relationship)
...
-- PART 4: Insert sample teams and officers if they don't exist
DO $$
DECLARE
    branch_record RECORD;
    team_counter INTEGER;
    officer_counter INTEGER;
BEGIN
    -- Create teams for each branch if they don't exist
    FOR branch_record IN 
        ...
        LIMIT 10
    LOOP
        -- Check if team exists for this branch
        IF NOT EXISTS (
            SELECT 1 FROM kastle_banking.collection_teams 
            WHERE branch_id = branch_record.branch_id
        ) THEN
            -- Insert a team for this branch
            INSERT INTO kastle_banking.collection_teams (
                ...
                'TEAM_' || branch_record.branch_id,
                branch_record.branch_name || ' Collection Team',
                'CALL_CENTER',
                branch_record.branch_id,
                true
            );
    ...
    -- Create officers for each team
    FOR team_counter IN 
        SELECT team_id 
        FROM kastle_banking.collection_teams 
        WHERE is_active = true
    LOOP
        -- Create 3-5 officers per team
        FOR officer_counter IN 1..floor(3 + random() * 3)::int LOOP
            INSERT INTO kastle_banking.collection_officers (
                ...
                'OFF_' || team_counter || '_' || officer_counter,
                'Officer ' || team_counter || '-' || officer_counter,
                'FIELD',
                team_counter,
                '+966' || floor(500000000 + random() * 100000000)::text,
                'officer' || team_counter || '_' || officer_counter || '@bank.com',
                'ACTIVE',
                1000000,
                2.5,
...
-- PART 5: Insert sample officer performance data
DO $$
DECLARE
    officer_record RECORD;
    date_counter DATE;
BEGIN
    -- Generate performance data for officers
...
-- PART 6: Refresh materialized view
SELECT kastle_banking.refresh_branch_comparison_data();

-- PART 7: Enable real-time
DO $$
BEGIN
    IF NOT EXISTS (
...
SELECT 'Setup Complete!' as status, 
       ...
       (SELECT COUNT(*) FROM kastle_banking.collection_teams) as teams_created,
       (SELECT COUNT(*) FROM kastle_banking.collection_officers WHERE status = 'ACTIVE') as active_officers;

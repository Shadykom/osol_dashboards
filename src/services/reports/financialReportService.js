...
class FinancialReportService {
  ...
  async getIncomeStatement(startDate, endDate, filters = {}) {
    try {
      ...
      let transactionQuery = supabaseBanking
        ...
        .select(`
          ...
          accounts!inner(
            ...
            customers!inner(customer_segment)
          )
        `)
      ...
      if (filters.segment && filters.segment !== 'all') {
        transactionQuery = transactionQuery.eq('accounts.customers.customer_segment', filters.segment);
      }
      ...
      if (filters.segment && filters.segment !== 'all') {
        loanQuery = loanQuery.eq('customers.customer_segment', filters.segment);
      }
  ...
  async getBalanceSheet(asOfDate, filters = {}) {
    try {
      ...
      if (filters.segment && filters.segment !== 'all') {
        accountQuery = accountQuery.eq('customers.customer_segment', filters.segment);
      }

-- Fix for Reports Page Errors (Unified Schema Version)
-- This script creates missing tables in kastle_banking schema

-- 1. Create missing collection tables in kastle_banking schema
...
CREATE TABLE IF NOT EXISTS kastle_banking.collection_contact_attempts (
...
CREATE TABLE IF NOT EXISTS kastle_banking.collection_risk_assessment (
...
CREATE TABLE IF NOT EXISTS kastle_banking.collection_settlement_offers (
...
-- Only create audit_log if it doesn't exist
CREATE TABLE IF NOT EXISTS kastle_banking.audit_log (
...
CREATE INDEX IF NOT EXISTS idx_collection_cases_case_status ON kastle_banking.collection_cases(case_status);
CREATE INDEX IF NOT EXISTS idx_collection_cases_bucket_id ON kastle_banking.collection_cases(bucket_id);
CREATE INDEX IF NOT EXISTS idx_collection_cases_assigned_to ON kastle_banking.collection_cases(assigned_to);

CREATE INDEX IF NOT EXISTS idx_daily_collection_summary_date ON kastle_banking.daily_collection_summary(summary_date);

CREATE INDEX IF NOT EXISTS idx_collection_contact_attempts_case_id ON kastle_banking.collection_contact_attempts(case_id);
CREATE INDEX IF NOT EXISTS idx_collection_contact_attempts_customer_id ON kastle_banking.collection_contact_attempts(customer_id);
...
-- 5. Create a view to map 'amount' to 'transaction_amount' for compatibility
...
-- 6. Add comments
COMMENT ON TABLE kastle_banking.collection_cases IS 'Main table for collection cases';
COMMENT ON TABLE kastle_banking.daily_collection_summary IS 'Daily summary of collection activities';
COMMENT ON TABLE kastle_banking.report_schedules IS 'Scheduled reports configuration';
COMMENT ON TABLE kastle_banking.collection_contact_attempts IS 'Records of contact attempts for collection cases';
COMMENT ON TABLE kastle_banking.collection_risk_assessment IS 'Risk assessment for collection cases';
COMMENT ON TABLE kastle_banking.collection_settlement_offers IS 'Settlement offers made to customers';
COMMENT ON TABLE kastle_banking.collection_officers IS 'Collection officers information';
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'kastle_banking' AND table_name = 'collection_cases') THEN
        RAISE NOTICE '✓ collection_cases table exists';
    ELSE
        RAISE WARNING '✗ collection_cases table missing';
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'kastle_banking' AND table_name = 'daily_collection_summary') THEN
        RAISE NOTICE '✓ daily_collection_summary table exists';
    ELSE
        RAISE WARNING '✗ daily_collection_summary table missing';
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'kastle_banking' AND table_name = 'report_schedules') THEN
        RAISE NOTICE '✓ report_schedules table exists';
    ELSE
        RAISE WARNING '✗ report_schedules table missing';
    END IF;
END $$;